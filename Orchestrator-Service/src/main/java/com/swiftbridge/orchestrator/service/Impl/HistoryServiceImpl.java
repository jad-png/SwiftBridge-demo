        ...
        return transactionHistoryRepository.findByTransactionId(transactionId)
                ...
                .map(history -> {
                    ...
                    if (history.getValidationErrors() != null && !history.getValidationErrors().isBlank()) {
                        try {
                            ...
                            validationErrors = java.util.Arrays.asList(
                                    mapper.readValue(history.getValidationErrors(),
                                            ConversionAuditDTO.ValidationError[].class));
                        } catch (Exception e) {
                            log.warn("Failed to parse validationErrors JSON for transaction {}: {}",
                                    history.getTransactionId(), e.getMessage());
                        }
                    }
                    return ConversionAuditDTO.builder()
                            ...
                            .requestTimestamp(
                                    history.getRequestTimestamp() != null ? history.getRequestTimestamp().toString()
                                            : null)
